...
get_googlemap <- function(
  ...
  llList <- RgoogleMaps::LatLon2XY(lon = center[1], lat = center[2], zoom = zoom)
  ll <- XY2LonLat(
    X = llList[[c(1,1)]], Y = llList[[c(1,2)]], zoom = zoom,
    x = llList[[c(2,1)]], y = llList[[c(2,2)]]
  )

  urList <- RgoogleMaps::LatLon2XY(lon = center[1], lat = center[2], zoom = zoom)
  ur <- XY2LonLat(llList[[c(1,1)]], llList[[c(1,2)]], zoom, llList[[c(2,1)]], llList[[c(2,2)]])
